...
const i18n = {
  en: {
    ...
    reflashTitle: "Already Flashed TouchPass Firmware?",
    reflashDesc: "Once flashed, ESP32-S3 operates as a USB HID keyboard and hides its COM port. To re-flash or update: Hold BOOT button, tap RESET, release BOOT button to enter Bootloader mode.",
...
- Verify USB serial hardware connection and guide initial fingerprint enrollment for 'y' + Enter AI CLI auto-approval.`,
  ...
  vi: {
    ...
    reflashTitle: "Đã từng nạp Firmware TouchPass?",
    reflashDesc: "Sau khi nạp, ESP32-S3 chuyển sang chế độ bàn phím USB HID và ẩn cổng COM Serial. Để nạp lại hoặc cập nhật: Giữ nút BOOT, bấm nhả nút RESET, sau đó thả nút BOOT để đưa mạch vào chế độ Bootloader.",
...
- Kiểm tra kết nối USB phần cứng và hướng dẫn đăng ký vân tay để tự động duyệt lệnh 'y' + Enter cho AI CLI.`,
  ...
  zh: {
    ...
    compatBoards: "兼容开发板",
    reflashTitle: "已经烧录过 TouchPass 固件？",
    reflashDesc: "烧录后 ESP32-S3 作为 USB HID 键盘运行并隐藏串口。要重新烧录或更新：按住 BOOT 按钮，点按 RESET，然后松开 BOOT 按钮进入 Bootloader 模式。",
...
- 验证 USB 串口硬件连接，并指引进行指纹录入，以测试 'y' + Enter 按键自动确认。`,
  ...
  ru: {
    ...
    reflashTitle: "Уже прошивали TouchPass?",
    reflashDesc: "После прошивки ESP32-S3 работает как клавиатура USB HID и скрывает COM-порт. Для перепрошивки или обновления: Зажмите кнопку BOOT, нажмите RESET, затем отпустите BOOT, чтобы войти в режим Bootloader.",
